...
public class Bus implements Comparable<Bus> {
  ...
  public int compareTo(Bus other) {
    int timeComparison = this.getArrivalTime().compareTo(other.getArrivalTime());

    if (timeComparison != 0) {
      // the time the bus arrives at the user's stop, earlier times less than later times
      return timeComparison;
    }
    //comparing based on  the route and tripName
    int nameComparison = this.route.getRouteAndTripName().compareTo(other.route.getRouteAndTripName());
    if (nameComparison != 0) {
      //comparing route and tripName in alphabetical order
      return nameComparison;
    }
    //comparing based on directions of the bus routes
