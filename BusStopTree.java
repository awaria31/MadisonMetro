...
public class BusStopTree {
  ...
  public static boolean isValidBST(Node<Bus> node) {
    ...
    private static boolean isValidBSTHelper(Node<Bus> node, Bus minValue, Bus maxValue) {
      ...
      if ((minValue != null && node.getValue().compareTo(minValue) <= 0) ||
        ...
        //return false if node's value is outside the valid range
        return false;
      }
      //use recursion to check left and right subtrees
      return isValidBSTHelper(node.getLeft(), minValue, node.getValue()) &&
              isValidBSTHelper(node.getRight(), node.getValue(), maxValue);
    }
  ...
  public boolean addBusHelper(Node<Bus> node, Bus bus) {
    //comparing the bus with the current node's value
    int compare = bus.compareTo(node.getValue());

    if (compare == 0) {
      return false; // if bus is already in the tree
    } else if (compare < 0) {
      //if bus is < the current node's value move on to left subtree
      if (node.getLeft() == null) {
        //if left child is null adding bus as a new left child
        node.setLeft(new Node<>(bus));
        return true;
      } else {
        //use recursion to add the bus to the left subtree
        return addBusHelper(node.getLeft(), bus);
      }
    } else {
      //if bus is greater than the current node's value, move to the right subtree
      if (node.getRight() == null) {
        //if right child is null adding bus as a new right child
        node.setRight(new Node<>(bus));
        return true;
      } else {
        ///use recursion to add the bus to the right subtree
  ...
  public Node<Bus> removeBusHelper(Node<Bus> node, Bus bus) {
    //base case, if the tree is empty or bus is null
    if (node == null) {
      return null;
    }
    //recursive case
    if (bus.compareTo(node.getValue()) < 0) {
      //bus is removed in the left subtree
      node.setLeft(removeBusHelper(node.getLeft(), bus));
    } else if (bus.compareTo(node.getValue()) > 0) {
      //bus is removed in the right subtree
      ...
      Node<Bus> minValueNode = findMin(node.getRight());
      node = new Node<>(minValueNode.getValue(), node.getLeft(), node.getRight());

      node.setRight(removeBusHelper(node.getRight(), minValueNode.getValue()));
    }
 ...
 */
    private Node<Bus> findMin(Node<Bus> node) {
  ...
  public boolean containsHelper(Node<Bus> node, Bus bus) {
    //base case, if  tree is empty/bus is not there
    if (node == null) {
      return false;
    }
    //recursive case
    ...
    if (comparisonResult == 0) {
      //finding the bus at the current node
      return true;
    } else if (comparisonResult < 0) {
      //the bus needing to be found is in the left subtree
      return containsHelper(node.getLeft(), bus);
    } else {
      // The bus needing to be found is in the right subtree
  ...
  protected Node<Bus> getFirstNodeAfter(LocalTime time, Node<Bus> node) {
    ...
    // Compare the arrival time of the bus at userStop with the given time
    int comparisonResult = node.getValue().getArrivalTime().compareTo(time);

    if (comparisonResult < 0) {
      // The soonest node at or after the time is in the right subtree
      return getFirstNodeAfter(time, node.getRight());
    } else {
      // Check both left and right subtrees and choose the best among them
      Node<Bus> leftNode = getFirstNodeAfter(time, node.getLeft());
      Node<Bus> rightNode = getFirstNodeAfter(time, node.getRight());

      if (leftNode == null && rightNode == null) {
        // Both subtrees do not have nodes at or after the time
        return comparisonResult >= 0 ? node : null;
      } else if (leftNode == null) {
        // Only the right subtree has a node at or after the time
        return rightNode;
      } else if (rightNode == null) {
        // Only the left subtree has a node at or after the time
        return leftNode;
      } else {
        // Both subtrees have nodes at or after the time; choose the soonest one
        return leftNode.getValue().getArrivalTime().compareTo(rightNode.getValue().getArrivalTime()) <= 0
                ? leftNode
                : rightNode;
      }
    }
  ...
  public static Node<Bus> getFirstBusHelper(Node<Bus> node) {
    //base case, ff the left child is null, this value is the leftmost node
    if (node.getLeft() == null) {
      return node;
    }
  ...
  public static Node<Bus> getLastBusHelper(Node<Bus> node) {
    //base case, if the right child is null, this is the rightmost node
    if (node.getRight() == null) {
      return node;
    }
